    ...
    CVs()
{
	...
	MString shaderStr;
	pfxShaders[0] = pfx.findPlug( "tubeShader", status );
	if( status == MS::kSuccess )
	{
		pfxShaders[0].getValue( shaderStr );
		if( shaderStr != "" )
			hasShader[0] = true;
	}
	pfxShaders[1] = pfx.findPlug( "leafShader", status );
	if( status == MS::kSuccess )
	{
		pfxShaders[1].getValue( shaderStr );
		if( shaderStr != "" )
			hasShader[1] = true;
	}
	pfxShaders[2] = pfx.findPlug( "petalShader", status );
	if( status == MS::kSuccess )
	{
		pfxShaders[2].getValue( shaderStr );
		if( shaderStr != "" )
			hasShader[2] = true;
	}
	...
	MPlug pFacing = pfx.findPlug( "tubeCameraFacing", status );
	if( status == MS::kSuccess )
		pFacing.getValue( camFacing[0] );
	pFacing = pfx.findPlug( "leafCameraFacing", status );
	if( status == MS::kSuccess )
		pFacing.getValue( camFacing[1] );
	pFacing = pfx.findPlug( "petalCameraFacing", status );
	if( status == MS::kSuccess )
		pFacing.getValue( camFacing[2] );

	// uniform width attributes
	bool uniformWidth[3] = { false, false, false };
	MPlug pUniform = pfx.findPlug( "tubeUniformWidth", status );
	if( status == MS::kSuccess )
		pUniform.getValue( uniformWidth[0] );
	pUniform = pfx.findPlug( "leafUniformWidth", status );
	if( status == MS::kSuccess )
		pUniform.getValue( uniformWidth[1] );
	pUniform = pfx.findPlug( "petalUniformWidth", status );
	if( status == MS::kSuccess )
		pUniform.getValue( uniformWidth[2] );

	// uniform rand attribute
	bool hasCurveID = false;
	MPlug curveID = pfx.findPlug( "curveID", status );
	if( status == MS::kSuccess )
			...
			for( unsigned lineOn( 0 ); lineOn < lines[ setOn ].length(); lineOn++ )
			{
				...
				if( MS::kSuccess == status )
				{
					...
					for( ; pOn < pfxVerts.length(); pOn++ )
					{
						...
						if( !hasShader[ setOn ] && pfxFlatness.length() )
						{
							hasFlatness = true;
						...
						*/
						if( !hasShader[ setOn ] && pfxColor.length() )
						{
							hasColor = true;
						...
						if( !hasShader[ setOn ] && pfxIncandescence.length() )
						{
						...
						if( !hasShader[ setOn ] && pfxTransparency.length() )
						{
							hasOpacity = true;
...
void liqRibPfxData::write()
{
	...
	for( unsigned setOn( 0 ); setOn < 3; setOn++ )
	{
		if( hasFeature[ setOn ] )
		{
			...
			{
				MString shaderStr;
				pfxShaders[ setOn ].getValue( shaderStr );
				RiSurface( ( RtToken )shaderStr.asChar(), RI_NULL );
			}
...
bool liqRibPfxData::writeNextGrain()
{
	...
	if( hasFeature[ grain ] )
	{
		...
		{
			MString shaderStr;
			pfxShaders[ grain ].getValue( shaderStr );
			RiSurface( ( RtToken )shaderStr.asChar(), RI_NULL );
		}
