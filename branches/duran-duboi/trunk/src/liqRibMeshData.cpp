...
void liqRibMeshData::write()
{
  if ( numPoints > 1 ) {
    ...
    if ( areaLight ) { // What happens if we're inside a motion block????? This whole approach of Liquid is flawed...
	...
	RtToken *tokenPtr = tokenArray.get();
	RtPointer *pointerPtr = pointerArray.get();
