...
int liqTokenPointer::set( const char * name, ParameterType ptype, bool asNurbs, bool asArray, bool asUArray, unsigned int arraySize )
{
	...
	m_isUArray = false;
	m_arraySize = 0;
	if( m_tokenFloats ) lfree( m_tokenFloats );
	m_tokenFloats = NULL;
	m_tokenSize = 0;
    }
    return 1;
...
int liqTokenPointer::adjustArraySize( unsigned int size )
{
    if( m_arraySize != size )
    {
    	// Only augment allocated memory if needed
	// Do not reduce it
...
void liqTokenPointer::getRiDeclare( char *declare  )
{
    ...
    case rString:
	    ...
	    break;
    case rFloat:
	    if( m_isUArray )  sprintf( declare, "%s float[%d]", StringDetailType[m_dType], m_uArraySize );
	    else    	    sprintf( declare, "%s float", StringDetailType[m_dType] );
	    break;
    case rHpoint:
    ...
    case rPoint:
	    if( m_isNurbs )
	    {
		if ( m_isArray ) sprintf( declare, "%s hpoint", StringDetailType[m_dType] );
		else sprintf( declare, "%s point", StringDetailType[m_dType] );
	    } else {
