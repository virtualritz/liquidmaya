...

{
  ...
  m_bDoRegionRender = (argData.isFlagSet( "-doRegion"));
	...
	if(argData.isFlagSet( "-renderFromFile"))
		argData.getFlagArgument("-renderFromFile", 0, m_bRenderFromFile);
	if(argData.isFlagSet( "-bucketFile"))
...

{
		...
		if(m_bGetRenderRegion)
	{
		...
		else MRenderView::startRegionRender (imgInfo.wo,imgInfo.ho,imgInfo.xo, imgInfo.yo, imgInfo.xo+imgInfo.width,imgInfo.yo+ imgInfo.height, false, true );
		vector<bucket*> buckets;
		bool bTestEnd;
		...
		if(m_bucketFile == ""){
			...
			if(tmp){
				char *tmpname = (char*)calloc(1,strlen(tmp)+14);
				strcat(tmpname,tmp);
				strcat(tmpname,"/liqRVXXXXXX");
				if(mktemp(tmpname))
					m_bucketFile = tmpname;
				free(tmpname);
			}
...
MStatus liqMayaRenderCmd::writeBuckets(const char* file, const vector<bucket*> &buckets,const imageInfo &info) const{
	...
	if(!fh)
	 {
		ERROR(MString("")+"[liqMayaRenderCmd] couldn't open "+file+" for writing");
		return MS::kFailure;
	}
	fwrite(&info,sizeof(imageInfo),1,fh);
	if(ferror(fh)){ ERROR("[liqMayaRenderCmd] error writing imageInfo"); fclose(fh); return MS::kFailure;}

	unsigned int numBuckets = buckets.size();
	fwrite(&numBuckets,sizeof(unsigned int),1,fh);
	if(ferror(fh)){ fclose(fh); return MS::kFailure;}
...
MStatus liqMayaRenderCmd::readBuckets(const char* file,vector<bucket*> &buckets, imageInfo &imgInfo) const{
	...
	for(unsigned int i =0; i < size; i++){
		...
		if(ferror(fh)) {
			ERROR(MString("[liqMayaRenderCmd] Error: failed reading data from ")+file);
			status = MS::kFailure;
			break;
		}
