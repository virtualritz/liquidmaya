...
public:
	...
	virtual	void		RiProjectionV(char *name,int n,char *tokens[],char *params[]);
	...
	virtual	void		RiImagerV(char *name,int n,char *tokens[],char *params[]);
	virtual	void		RiQuantize(char * type,int one,int qmin,int qmax,float ampl);
	virtual	void		RiDisplayV(char *name,char * type,char * mode,int n,char *tokens[],char *params[]);
	virtual	void		RiDisplayChannelV(char * channel,int n,char *tokens[],char *params[]);

	virtual	void		RiHiderV(char * type,int n,char *tokens[],char *params[]);
	virtual	void		RiColorSamples(int N,float *nRGB,float *RGBn);
	virtual	void		RiRelativeDetail(float relativedetail);
	virtual	void		RiOptionV(char *name,int n,char *tokens[],char *params[]);
	...
	virtual	void		*RiLightSourceV(char *name,int n,char *tokens[],char *params[]);
	virtual	void		*RiAreaLightSourceV(char *name,int n,char *tokens[],char *params[]);

	virtual	void		RiIlluminate(void *light,int onoff);
	virtual	void		RiSurfaceV(char *name,int n,char *tokens[],char *params[]);
	virtual	void		RiAtmosphereV(char *name,int n,char *tokens[],char *params[]);
	virtual	void		RiInteriorV(char *name,int n,char *tokens[],char *params[]);
	virtual	void		RiExteriorV(char *name,int n,char *tokens[],char *params[]);
	...
	virtual	void		RiDeformationV(char *name,int n,char *tokens[],char *params[]);
	virtual	void		RiDisplacementV(char *name,int n,char *tokens[],char *params[]);
	...
	virtual	void		RiAttributeV(char *name,int n,char *tokens[],char *params[]);

	
	virtual	void		RiPolygonV(int nvertices,int n,char *tokens[],char *params[]);
	virtual	void		RiGeneralPolygonV(int nloops,int *nverts,int n,char *tokens[],char *params[]);
	virtual	void		RiPointsPolygonsV(int npolys,int *nverts,int *verts,int n,char *tokens[],char *params[]);
	virtual	void		RiPointsGeneralPolygonsV(int npolys,int *nloops,int *nverts,int *verts,int n,char *tokens[],char *params[]);
	virtual	void		RiBasis(float ubasis[][4],int ustep,float vbasis[][4],int vstep);
	virtual	void		RiPatchV(char * type,int n,char *tokens[],char *params[]);
	virtual	void		RiPatchMeshV(char * type,int nu,char * uwrap,int nv,char * vwrap,int n,char *tokens[],char *params[]);
	virtual	void		RiNuPatchV(int nu,int uorder,float *uknot,float umin,float umax,int nv,int vorder,float *vknot,float vmin,float vmax,int n,char *tokens[],char *params[]);
	virtual	void		RiTrimCurve(int nloops,int *ncurves,int *order,float *knot,float *amin,float *amax,int *n,float *u,float *v,float *w);

	virtual	void		RiSphereV(float radius,float zmin,float zmax,float thetamax,int n,char *tokens[],char *params[]);
	virtual	void		RiConeV(float height,float radius,float thetamax,int n,char *tokens[],char *params[]);
	virtual	void		RiCylinderV(float radius,float zmin,float zmax,float thetamax,int n,char *tokens[],char *params[]);
	virtual	void		RiHyperboloidV(float *point1,float *point2,float thetamax,int n,char *tokens[],char *params[]);
	virtual	void		RiParaboloidV(float rmax,float zmin,float zmax,float thetamax,int n,char *tokens[],char *params[]);
	virtual	void		RiDiskV(float height,float radius,float thetamax,int n,char *tokens[],char *params[]);
	virtual	void		RiTorusV(float majorrad,float minorrad,float phimin,float phimax,float thetamax,int n,char *tokens[],char *params[]);
	virtual	void		RiProcedural(char * data,float *bound,void (*subdivfunc)(char *,float),void (*freefunc)(char *));
	virtual	void		RiGeometryV(char * type,int n,char *tokens[],char *params[]);

	virtual	void		RiCurvesV(char * degree,int ncurves,int nverts[],char * wrap,int n,char *tokens[],char *params[]);
	virtual	void		RiPointsV(int npts,int n,char *tokens[],char *params[]);
	virtual	void		RiSubdivisionMeshV(char * scheme,int nfaces,int nvertices[],int vertices[],int ntags,char * tags[],int nargs[],int intargs[],float floatargs[],int n,char *tokens[],char *params[]);
	virtual	void		RiBlobbyV(int nleaf,int ncode,int code[],int nflt,float flt[],int nstr,char *str[],int n,char *tokens[],char *params[]);
	...
	virtual	void		RiMakeTextureV(char *pic,char *tex,char * swrap,char * twrap,float (*filterfunc)(float,float,float,float),float swidth,float twidth,int n,char *tokens[],char *params[]);
	virtual	void		RiMakeBumpV(char *pic,char *tex,char * swrap,char * twrap,float (*filterfunc)(float,float,float,float),float swidth,float twidth,int n,char *tokens[],char *params[]);
	virtual	void		RiMakeLatLongEnvironmentV(char *pic,char *tex,float (*filterfunc)(float,float,float,float),float swidth,float twidth,int n,char *tokens[],char *params[]);
	virtual	void		RiMakeCubeFaceEnvironmentV(char *px,char *nx,char *py,char *ny,char *pz,char *nz,char *tex,float fov,float (*filterfunc)(float,float,float,float),float swidth,float twidth,int n,char *tokens[],char *params[]);
	virtual	void		RiMakeShadowV(char *pic,char *tex,int n,char *tokens[],char *params[]);

	virtual	void		RiErrorHandler(void (*handler)(int,int,char *));

	virtual	void		RiArchiveRecord(char * type,char *format,va_list args);
	virtual	void		RiReadArchiveV(char *filename,void (*callback)(const char *),int n,char *tokens[],char *params[]);
...
private:
	void				writePL(int,char *[],char *[]);
	void				writePL(int numVertex,int numVarying,int numFaceVarying,int numUniform,int,char *[],char *[]);
